...
public class StudentMan extends JPanel{
	...
	void init() {
		...
		JButton jButton3=new JButton("更改学生");
		...
		Object columns[] ={"序号","学生学号","姓名","所在学院","专业","性别","入学年份"};
		...
		cmb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(Temp.getSelectedIndex()!=0) {
					...
					Tools.upDateComboBox("select * from major where college='"+t+"'", cmb1,"请选择专业","proname");
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(name.equals("")) {
					...
					while(true) {
						...
						if(rs!=-1) {
							String data11[]= {num};
							ResultSet rs_1=Mysqld.QueryData("select * from student where id=?",data11);
		...
		jButton.addActionListener(new ActionListener() {
			...
			if(numString.equals("")) {
				...
				ResultSet rs = Mysqld.QueryData("select * from student", null);
				...
				ResultSet rs = Mysqld.QueryData("select * from student where id=?", data);
				...
				rs = Mysqld.QueryData("select * from student where id=?", data);
		...
		jButton2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String numString=	jTextField2.getText();
				if(numString.equals("")) {
					Tools.messageWindows("请输入学生学号");
				}else {
					
					String data[]= {numString};
					
					int sta=Mysqld.upDate("DELETE from student where id =?", data);
					if(sta==0) {
						Tools.messageWindows("删除学生失败,请检查学号");
					}
					if(sta==-1) {
		...
		jButton3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(name.equals("")) {
					...
					if(num==0) {
						Tools.messageWindows("请检查学生学号是否正确");
						
					}
					if(num==-1) {
						Tools.messageWindows("请检查学生学号格式是否正确！");
						
					}
					if(num==1) {
						Tools.messageWindows("学生信息更改成功");
